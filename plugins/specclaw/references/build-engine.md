...
```
1. specclaw-build setup .specclaw <change>              → config JSON (branch created if needed)
2. specclaw-parse-tasks --status pending tasks.md        → JSON task array
3. For each wave (sequential):
   a. specclaw-parse-tasks --wave N --status pending     → tasks in this wave
   b. For each task (parallel up to parallel_tasks limit):
      - specclaw-build-context .specclaw <change> <task_id>  → context payload (stdout)
      - specclaw-update-task-status tasks.md T1 in_progress
      - sessions_spawn with context payload
      - On completion:
        - specclaw-update-task-status tasks.md T1 complete|failed
        - specclaw-build commit .specclaw <change> T1 "title" files...
4. specclaw-build finalize .specclaw <change>            → summary JSON (tests, lint, merge)
5. specclaw-update-status .specclaw                      → regenerate STATUS.md dashboard
```
...
All scripts live in `plugins/specclaw/bin/` and require only bash, coreutils, and git. `jq` is optional (grep fallbacks exist).

### `specclaw-build` — Setup, Commit, Finalize

```
specclaw-build <subcommand> [args]
```
...
### `specclaw-parse-tasks` — Task Parser

```
specclaw-parse-tasks [OPTIONS] <tasks.md>
```
...
### `specclaw-build-context` — Context Payload Builder

```
specclaw-build-context <specclaw_dir> <change_name> <task_id>
```
...
- `.specclaw/changes/<change>/tasks.md` — parsed via `specclaw-parse-tasks` internally
...
### `specclaw-update-task-status` — Status Updater

```
specclaw-update-task-status [OPTIONS] <tasks.md> <task_id> <new_status>
specclaw-update-task-status [OPTIONS] <tasks.md> --batch T1:complete T2:failed ...
```
...
### `specclaw-update-status` — Dashboard Generator

```
specclaw-update-status <specclaw_dir>
```
...
### `specclaw-build finalize` Output
...
The `specclaw-build-context` script enforces this by building each payload independently from source files on disk.
...
1. `specclaw-update-task-status` marks the task as `[!] failed`
...
# specclaw-build setup detects existing branch:
...
```bash
specclaw-update-task-status --batch --log tasks.md T1:complete T2:failed T3:complete
```
...
```
specclaw-build setup   → git checkout -b specclaw/<change>  (or switch if exists)
specclaw-build commit  → git add <files> && git commit -m "specclaw(<change>): T1 — title"
  ... repeat per task ...
specclaw-build finalize → git checkout main && git merge --no-ff specclaw/<change>
...
```
specclaw-build setup   → no branch operations
specclaw-build commit  → git add <files> && git commit (on current branch)
specclaw-build finalize → run checks only, no merge step
```
...
**Symptom:** `specclaw-parse-tasks` outputs invalid JSON or the build engine can't read task data.
...
**Symptom:** `specclaw-build finalize` reports merge conflict error.
...
**Cause:** `specclaw-build-context` includes full `spec.md` and `design.md` plus file contents.
...
**What happens:**
- `specclaw-build setup` detects the existing branch and switches to it (`branch_existed: true`)
- `specclaw-parse-tasks --status pending` returns only incomplete tasks
