...
- **FR15** — After renaming an **active** folder that has a `state.json`, the recorded `change` field is refreshed by re-invoking `specclaw-set-phase` at the change's current phase and status. `specclaw-set-phase` remains the only writer of phase state; this script never edits `state.json` directly.
- **FR15a** — The refresh must round-trip every field the phase record carries — `verdict`, `url`, `tasks`, `branch` — not merely phase and status. `specclaw-set-phase` rebuilds the record from its arguments, so omitting a field *deletes* it: a verified change would silently lose its `PASS` verdict. Round-tripping them also makes the rebuilt record byte-identical, which is the exact condition under which `set-phase` preserves the original `at` timestamp instead of stamping a fresh one.
- **FR15b** — Archived folders are **skipped** by the FR15 refresh. `specclaw-set-phase` resolves `<specclaw_dir>/changes/<change>`, so refreshing an archived folder would require passing `archive/<name>` as the change argument and would record `"change": "archive/001-foo"` — a path, not the change's identity, and inconsistent with the bare name the archive skill writes before the move. Archived state is terminal and read by nothing: `specclaw-update-status` documents that archived rows are *"deliberately not read here — nothing under archive/ can affect this listing"*. Skipping is both simpler and more correct than writing a wrong-looking value.
